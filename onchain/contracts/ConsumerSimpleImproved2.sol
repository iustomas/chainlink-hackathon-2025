...
contract ConsumerSimpleImproved2 is FunctionsClient {
    ...
    function execute(string calldata signature, uint256 timestamp) external {
        FunctionsRequest.Request memory req;
        // Hardcode the source to use the consumer-simple.js script
        string memory source = "// Call escalate-to-human-lawyer endpoint with required params\nconst response = await Functions.makeHttpRequest({\n  url: 'https://tomas-web3-api-ex5ksmcdeq-uc.a.run.app/tomas/escalate-to-human-lawyer',\n  method: 'POST',\n  timeout: 10000,\n  data: {\n    caseId: args[0],\n    contractAddress: args[1],\n    signature: args[2],\n    timestamp: args[3],\n  },\n});\nif (response.error) { throw Error(JSON.stringify(response)); }\nreturn Functions.encodeString(JSON.stringify(response.data));";
        req.initializeRequestForInlineJavaScript(source);
        // Prepare arguments: [caseId, contractAddress, signature, timestamp]
        string[] memory args = new string[](4);
        args[0] = toAsciiString(msg.sender); // caseId as user address
        args[1] = toAsciiString(address(this)); // contract address
        args[2] = signature;
        args[3] = timestamp.toString();
        req.setArgs(args);
